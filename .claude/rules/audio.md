...
- Narration uses distance-based volume: `activateNarration` resumes or starts, `deactivateNarration` pauses (preserving playhead), `restartNarration` seeks to 0 (available but not currently used), `setNarrationVolume` is called per-frame from `main.js`.
