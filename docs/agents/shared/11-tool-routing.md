...
5. Use validation tools such as `playwright`, `responsiveness-check`, or `ux-audit` only after code or content changes, following `docs/agents/shared/09-validation-checklist.md`.
...
## MCP routing by phase

| Phase | Primary source of truth | MCP/tool route | Why |
| --- | --- | --- | --- |
...
| UX and browser validation | Running app behavior | `playwright`, `responsiveness-check`, `ux-audit` | Confirms interaction, responsive layout, and regression risk; use `09-validation-checklist.md` for the exact ladder |
| Design inspection | Real design artifact | `figma` | Only relevant when an actual file/frame/node exists |

## When not to use each MCP
...
Do not use it as a primary code-understanding tool. It validates browser behavior; it does not replace reading `src/main.js`, subsystem modules, or config files.
...
| Tool/MCP | If unavailable | Fallback |
| --- | --- | --- |
...
| `playwright` | Browser automation unavailable | Use static review plus manual QA instructions; record the validation gap |
